...
const PlayerStatsChart: React.FC<PlayerStatsChartProps> = ({
  ...
  const calculatePlayerStats = (): PlayerStat[] => {
    return players.map((player, index) => {
      ...
      const pointsLeft = Math.max(0, (game.forPoints - 1) - totalScore);
  ...
  // Use the same coordinate system as bars: proportional height from bottom
  const targetLinePosition = (game.forPoints / maxScore) * chartHeight;
